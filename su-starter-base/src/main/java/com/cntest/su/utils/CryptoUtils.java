...
import com.cntest.su.constant.Algorithm;
...
public class CryptoUtils {
  ...
  public static String encodeBase64(String content) {
    try {
      return new String(encodeBase64(content.getBytes(Encoding.UTF8)));
  ...
  public static String decodeBase64(String content) {
    try {
      return new String(decodeBase64(content.getBytes(Encoding.UTF8)));
    } catch (UnsupportedEncodingException e) {
  ...
  public static String aesEncrypt(String content, String password) {
    try {
      byte[] raw = password.getBytes(Encoding.UTF8);
      SecretKeySpec key = new SecretKeySpec(raw, "AES");
      Cipher cipher = Cipher.getInstance("AES/ECB/PKCS5Padding");
      cipher.init(Cipher.ENCRYPT_MODE, key);
      byte[] encrypted = cipher.doFinal(content.getBytes(Encoding.UTF8));
  ...
  public static String aesDecrypt(String content, String password) {
    try {
      ...
      return new String(original, Encoding.UTF8);
    } catch (Exception e) {
  ...
  public static String sign(String srcString, String algorithm, PrivateKey privateKey) {
    try {
      ...
      rsa.update(srcString.getBytes(Encoding.UTF8));
      byte[] sig = rsa.sign();
